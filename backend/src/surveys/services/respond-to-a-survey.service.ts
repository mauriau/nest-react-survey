...
export class RespondToASurveyService {
    ...
    async respondToASurvey(createResponseSurveyDto: CreateResponseSurveyDto, participant: User): Promise<SurveyResponse>{
        const {surveyId, choiceIds} = createResponseSurveyDto

        const survey = await this.surveyRepository.findOne({where: {id: surveyId},  relations: ['choices']})
        if(!survey){
            throw new NotFoundException("This Survey doesnt exist")
        }
        const surveyChoices = survey.choices.map((choice) => choice.id)
        if(survey.singleResponse && surveyChoices.length > 1){
